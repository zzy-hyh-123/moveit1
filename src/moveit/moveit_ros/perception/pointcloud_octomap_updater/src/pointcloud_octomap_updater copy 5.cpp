...
void PointCloudOctomapUpdater::cloudMsgCallback(const sensor_msgs::PointCloud2::ConstPtr& cloud_msg)
{
  ...
  // 核心参数（根据你的场景调整）
  const double ROBOT_CLEAR_RADIUS = 0.15;  // 机械臂周围15cm内所有体素强制清除
    ...
    if (tf_buffer_)
    {
      try
      {
        tf2::fromMsg(tf_buffer_->lookupTransform(monitor_->getMapFrame(), cloud_msg->header.frame_id,
                                                 ...
                                                 ros::Duration(0.1)), // 增加TF缓存，解决运动时不同步
                     map_h_sensor);
      }
  ...
  try
  {
    ...
    else
    {
      ...
      // 【关键修复2】强制清除机械臂周围15cm内的所有体素
      // 彻底解决自碰撞误报问题
      for (const octomap::OcTreeKey& model_cell : model_cells)
      {
        octomap::point3d model_point = tree_->keyToCoord(model_cell);
        
        // 遍历机械臂连杆周围15cm范围内的所有体素
        octomap::point3d bbx_min(
          model_point.x() - ROBOT_CLEAR_RADIUS,
          model_point.y() - ROBOT_CLEAR_RADIUS,
          model_point.z() - ROBOT_CLEAR_RADIUS
        );
